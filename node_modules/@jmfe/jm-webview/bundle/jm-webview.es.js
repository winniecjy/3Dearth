import { assign, getAppVersion, isAndroid, isApp, isArray, isIOS, isIPhone, isIpad, isMobile, isObject, isString, isTablet, isWebview, serialize, versionCompare } from '@jmfe/jm-common';
...
function getHomeSchema() {
	if (isAndroid()) return buildSchema('360buy', { type: 1 });else if (isIOS()) return buildSchema('virtual', { category: 'jump', des: 'HomePage' });
}
...
function processingChannel(inputChannels) {
	...
	if (inputChannels) {
		if (isIOS()) {
			...
			var appVer = getAppVersion('jd');

			if (isMobile() && isApp('jd') && versionCompare(appVer, '4.4.0') >= 0 && versionCompare(appVer, '5.0.0') < 0) {
	...
	if (isInputParamObjLegal(inputParamObj)) {
		...
		try {
			if (Object.prototype.toString.call(inputParamObj) === "[object Object]") {
				assign(targetParamObj, inputParamObj);
			}
			// 处理回调
...
function setShareInfo(params) {
	...
	try {
		...
		if (isAndroid()) {
			...
			// jdApp 5.0及以上版本
			var appVer = getAppVersion('jd');
			if (isIPhone() && versionCompare(appVer, '5.0.0') >= 0 || isIpad() && versionCompare(appVer, '3.6.0') >= 0) {
...
function configBtnSet(options) {
	...
	if (!isApp('jd')) return;
	var params = assign(defaults, options);
...
export { nativeCall, buildSchema, getProductSchema, toProduct, getShopSchema, toShop, getHomeSchema, toHome, getCouponCenterSchema, toCouponCenter, getSecKillSchema, toSecKill, toFlashSale, getOrderListSchema, toOrderList, toLogin, openNewWebView, getDiscoverySchema, toDiscovery, getArticleSchema, toArticle, getAuthorSchema, toAuthor, getInventorySchema, toInventory, getInventoryDetailSchema, toInventoryDetail, getCartSchema, toCart, refreshCart, showCartButton, getLiveCastListSchema, toLiveCastList, getLiveCastPreviewSchema, toLiveCastPreview, getLiveCastRoomSchema, toLiveCastRoom, setShareInfo, callSharePane, sendDirectShare, setNaviBackground, configBtnClear, configBtnSet, configBtnSince610, showCloseBtn, enableTransparent, setTitle, showNativeBarcodeScan, showNativeUpload, XViewClose, XViewConfigCloseButton, JDPaySdk_pay, JDPaySdk_pop, saveImageToPhoteAlbum, bindSocialAccountWithJsonString, selectImage, closeLivesWindow, goWxUrl, getFingerInfo, getNavHeight, sourceIsReady };
